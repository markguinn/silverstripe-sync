...
@synthesize model, config, batch, delegate;
...
- (NSURLConnection*)process
{
    ...
    for (NSManagedObject *rec in recs) 
    {
        ...
        if (remoteID > 0)
        {
            ...
            else 
            {
                ...
                NSDictionary *checkMe = [[NSDictionary alloc] initWithObjectsAndKeys:
                                         [NSNumber numberWithInt:remoteID], @"ID",
                                         [NSNumber numberWithInt:timeStamp], @"TS",
                                         nil];
    ...
    NSError *error;
    [dataOut setObject:[NSJSONSerialization dataWithJSONObject:toInsert options:kNilOptions error:&error] forKey:@"insert"];
    [dataOut setObject:[NSJSONSerialization dataWithJSONObject:toCheck options:kNilOptions error:&error] forKey:@"check"];
...
{
    NSLog(@"Request failed: %@ (state=%d)", error, _state);
...
{
    //NSLog(@"Request finished: %d", _state);
    ...
    if (_state == SYNCREQUEST_STATE_CHECK)
    {
        ...
        {
            NSLog(@"Request failed. reason=%@", [dataIn valueForKey:@"statusMessage"]);
        ...
        if ([remoteUpdate count] > 0)
        {
            for (NSDictionary* remoteRec in remoteUpdate)
            {
                ...
                if (localRec != nil) 
                {
                    NSLog(@"Updating record #%@", [localRec valueForKey:@"remoteID"]);
        ...
        if ([_localDelete count] > 0)
        {
            for (id remoteID in _localDelete)
            {
                ...
                if (localRec != nil) 
                {
                    NSLog(@"Deleting record #%@", [localRec valueForKey:@"remoteID"]);
        ...
        if ([remoteInsert count] > 0)
        {
            for (NSDictionary* remoteRec in remoteInsert)
            {
                ...
                [self setValuesForLocal:localRec fromRemote:remoteRec];
                NSLog(@"Inserting record #%@", [localRec valueForKey:@"remoteID"]);
            }
        }
